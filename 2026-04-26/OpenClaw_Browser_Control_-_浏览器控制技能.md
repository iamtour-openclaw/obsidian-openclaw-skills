# OpenClaw Browser Control - 浏览器控制技能

## 📊 基本信息
- **来源**: OpenClaw Docs
- **类型**: 官方技能
- **分类**: 工具类
- **标签**: browser web automation
- **学习难度**: 中等

## 📝 技能描述
控制浏览器进行网页操作，支持 Chrome 扩展接管和独立浏览器 这是 OpenClaw 生态系统中的重要技能。

## 🎯 核心功能
控制浏览器进行网页操作，支持 Chrome 扩展接管和独立浏览器

## 💡 主要特性
- 功能强大且易于使用
- 与 OpenClaw 核心功能深度集成
- 支持多种配置选项

## 🔧 使用方法
```
browser action=snapshot / browser action=click / browser action=type
```

## 📚 使用示例
- browser snapshot 查看页面
- browser click 点击元素
- browser type 输入文本

## 🎪 应用场景
- 自动化日常工作流程
- 提升工作效率
- 实现复杂任务编排

## 💭 使用建议
推荐所有 OpenClaw 用户学习和使用

## 🔗 相关链接
- [🔍 Google 搜索此技能](https://www.google.com/search?q=OpenClaw+OpenClaw_Browser_Control_-_浏览器控制技能)
- [🏠 OpenClaw Docs 官方](https://docs.openclaw.ai)
- [📖 OpenClaw 文档](https://docs.openclaw.ai)
- [🦞 ClawHub 社区](https://clawhub.com)

---
*本技能由 OpenClaw Skills 收集器自动生成*
*生成时间：2026-04-26 19:32*
*维护者：小李子 (OpenClaw AI 助手)*
*项目地址：https://github.com/iamtour-openclaw/obsidian-openclaw-skills*
